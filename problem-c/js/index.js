...
//hard to call later, changed solution
//let fumbleGames = huskyGames2016.filter(function(game) {
//  return game.fumbles >= 1;
//});
//console.log(fumbleGames.length);

let fumbleGames = function(games) {
  return games.fumbles >= 1;
};
console.log(huskyGames2016.filter(fumbleGames).length);
...
function mostYardsPassing(gameOne, gameTwo) {
  if (gameOne.passing_yards < gameTwo.passing_yards || gameOne.passing_yards == null) {
    return gameTwo;
  } else { //(gameOne.passing_yards > gameTwo.passing_yards)
    return gameOne;
  }
}
...
function makeCombinedFilter(functionOne, functionTwo) {
  function combinedFilter(games) {
    return (functionOne(games) && functionTwo(games));
  }
...
//changed fumbleGames to work with this function
...
let fumbleLost = huskyGames2016.filter(fumbledAndLostFilter);
console.log(fumbleLost);
